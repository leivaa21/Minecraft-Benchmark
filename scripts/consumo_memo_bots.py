...
def main():
    ...
    filenameBots = "txts/yardstick_bot_connected.txt"
    ...
    with open(filenameBots) as f:
        lineBots = f.readlines()

    index = 0
    for line in lineBots:
        vectorLinea = line.split(' @')
        if index == 0:
            bots.append(0)
        else:
            bots.append(float(vectorLinea[0]))
        index += 1
        lineaAnterior = float(vectorLinea[0])

    plt.plot(x, memoria, c="red", alpha=0.5, label="RAM")
    plt.plot(x, bots, c="orange", alpha=0.5, label="Núm bots")
    plt.legend()

    plt.annotate("50", (748, 51))
